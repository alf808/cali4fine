...
ggplot(aes(x = contb_receipt_dt, y = n), data = testsub) +
  #geom_line() +
  geom_point() +
  geom_smooth() +
...
ggplot(aes(x = cand_nm, y = amt_rcvd), data = temp.subset) +
  ...
  coord_flip() +
  scale_y_continuous(limits = c(0, max(temp.subset$amt_rcvd)+250000), 
                     label = comma) +
  scale_fill_manual(values = c("blue", "green", "red")) +
  #geom_text(aes(label = amt_rcvd, stat = "identity", size = 3, hjust = -.2)) +
