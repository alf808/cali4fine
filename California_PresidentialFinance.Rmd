...
# api.key.install("ce314bcf6a442a68173360668caa80c7174bc96f", file="key.rda")
```
...
cta <- subset(capcf, contb_receipt_amt > 0)
cta <- cta %>%
...
ggplot(aes(x = contbr_city, y = contb_receipt_amt, color = cand_party), data = subset(capcf, contb_receipt_amt > 0 & contbr_city %in% cta$contbr_city)) +
  geom_bar(stat = "identity") +
  xlab("Cities") +
...
I did a correlation test between the date and amount. At -0.2374, I doubt there's a correlation other than the fact that time is progressive and events occur as time progresses.
...
I was trying to show the cumulative amount as time progresses. I could not separate by parties.
...
```{r echo=FALSE, point_plot_donations_time}
...
#ggplot(aes(region, value), data = ca_income)
...
### alameda most diverse county
#### http://www.theatlantic.com/national/archive/2014/04/mapping-racial-diversity-by-county/361388/
```{r echo=FALSE, alameda_zoom}
# capcf.contbr.by_zip <- na.omit(capcf.contbr.by_zip)

alameda.zips <- subset(cazips, county.name == "alameda")

capcf.alameda <- capcf %>%
  filter(contb_receipt_amt > 0 & contbr_zip %in% alameda.zips$region) %>%
  group_by(contbr_zip) %>%
  summarize(value = sum(contb_receipt_amt)) %>%
  arrange(desc(value))

capcf.alameda$region <- capcf.alameda$contbr_zip

cali_zip <- ZipChoropleth$new(capcf.alameda)
cali_zip$legend = 'Total Donations'
cali_zip$ggplot_scale = scale_fill_brewer(palette = "PuRd", drop=FALSE)
cali_zip$set_zoom_zip(state_zoom="california", county_zoom="alameda",
                      msa_zoom=NULL,
                      zip_zoom=NULL)
zAlameda <- cali_zip$render()
zAlameda
```

## testing

```{r echo=FALSE, message=FALSE, warning=FALSE, Experiment_Plot_Three}
cta <- capcf %>%
...
ggplot(aes(x = ReorderSize(contbr_city), y = contb_receipt_amt), data = subset(capcf, contb_receipt_amt > 0 & contbr_city %in% cta$contbr_city)) +
  geom_bar(stat = "identity", aes(fill = cand_party, group = cand_party), position = "dodge") +
  xlab("Cities") +
  ylab("Total amount of contributions") +
  ggtitle("Top cities by total amounts") +
  coord_flip() +
  scale_fill_manual(values = c("blue", "green", "red"))

anothertemp <- subset(capcf, contb_receipt_amt > 0 & capcf$contbr_city %in% cta$contbr_city)

ggplot(aes(x = contbr_city, y = contb_receipt_amt), data = anothertemp) +
  stat_summmary(aes(color = cand_party), fun.y = "sum") +
  geom_bar(stat = "identity") +
  coord_flip() +
  scale_fill_manual(values = c("blue", "green", "red"))

ggplot(aes(x = ReorderSize(contbr_city), fill = cand_party), data = anothertemp) +
